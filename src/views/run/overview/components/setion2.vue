			...
			<a-tabs default-active-key="1" size="small" :tab-bar-style="{ marginBottom: '24px', paddingLeft: '16px' }">
				<template #leftExtra>
					<div class="extra-wrapper">
						<div class="title">用户使用各功能人数统计</div>
					</div>
				</template>
				<template #rightExtra>
					<div class="extra-wrapper">
				...
				<a-tab-pane loading="true" tab="" key="1">
					<bars :data="state.barData" title="使用人数" id="sales2" />
				</a-tab-pane>
				<!-- <a-tab-pane tab="折线图" key="2">
					<chart-lines :data="state.barData" title="使用人数" id="visits2" />
				</a-tab-pane> -->
			</a-tabs>
		</div>
	</a-card>
...
<script lang="ts" setup name="overview">
	import {fun} from '@/api/run'
	import { ref, reactive, onMounted } from 'vue'
	import dayjs from 'dayjs'
	import Bars from '@/components/Charts/Bars.vue'
	import ChartLines from '@/components/Charts/Lines.vue'
	...
	const getData = () => {
		...
		fun(params).then((res) => {
			...
			date.forEach( date => {
				arr.forEach( item  => {
					const data = res.data.find( resDate => resDate.date === date && item.funId === resDate.funId);
					if (data) {
