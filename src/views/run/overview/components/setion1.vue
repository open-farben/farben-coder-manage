<template>
	<a-card :loading="loading" :bordered="false" :body-style="{ padding: '0', marginBottom: '20px' }">
		<div class="salesCard">
			<a-tabs default-active-key="1" size="small" :tab-bar-style="{ marginBottom: '24px', paddingLeft: '16px' }">
				<template #leftExtra>
					<div class="extra-wrapper">
						<div class="title">用户使用插件人数统计</div>
					</div>
				</template>
				<template #rightExtra>
					<div class="extra-wrapper">
				...
				<a-tab-pane loading="true" tab="" key="1">
					<stacked :data="state.barData" title="使用人数" id="sales1" />
				</a-tab-pane>
				<!-- <a-tab-pane tab="折线图" key="2">
					<chart-line :data="state.barData" title="使用人数" id="visit1" />
				</a-tab-pane> -->
			</a-tabs>
		</div>
	</a-card>
...
<script lang="ts" setup name="overview">
	...
	const getData = () => {
		...
		dashboard(params).then((res) => {
			...
			for(let i in res.data){
				for(let x in list){
					if(res.data[i].date === list[x].date){
						list[x].value = res.data[i].value;
						list[x].duty = res.data[i].duty;
						list[x].type = res.data[i].type;
					}
				}
			}
			state.barData = list || []*/
			...
			date.forEach( arr => {
				obj[arr] = []
				const data = res.data.filter( resDate => resDate.date === arr);
				if (data.length > 0) {
					obj[arr] = data;
				}else{
					obj[arr].push({date: arr})
				}
			})
